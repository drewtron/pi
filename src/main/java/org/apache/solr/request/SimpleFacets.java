...
public class SimpleFacets {
    ...
    public NamedList<Integer> getTermCounts(String field) throws IOException {
        int offset = params.getFieldInt(key, FacetParams.FACET_OFFSET, 0);
        int limit = params.getFieldInt(key, FacetParams.FACET_LIMIT, 100);
        if (limit == 0) return new NamedList<Integer>();
        Integer mincount = params.getFieldInt(key, FacetParams.FACET_MINCOUNT);
        if (mincount==null) {
            Boolean zeros = params.getFieldBool(key, FacetParams.FACET_ZEROS);
            // mincount = (zeros!=null && zeros) ? 0 : 1;
            mincount = (zeros!=null && !zeros) ? 1 : 0;
            // current default is to include zeros.
        }
        boolean missing = params.getFieldBool(key, FacetParams.FACET_MISSING, false);
        // default to sorting if there is a limit.
        String sort = params.getFieldParam(key, FacetParams.FACET_SORT, limit>0 ? FacetParams.FACET_SORT_COUNT : FacetParams.FACET_SORT_INDEX);
        String prefix = params.getFieldParam(key,FacetParams.FACET_PREFIX);
        ...
        String method = params.getFieldParam(key, FacetParams.FACET_METHOD);
        ...
        if (params.getFieldBool(key, GroupParams.GROUP_FACET, false)) {
    ...
    public void getFacetDateCounts(String dateFacet, NamedList<Object> resOuter)
        ...
        final String startS
                = required.getFieldParam(key,FacetParams.FACET_DATE_START);
        final Date start;
        try {
        ...
        final String endS
                = required.getFieldParam(key,FacetParams.FACET_DATE_END);
        ...
        final String gap = required.getFieldParam(key,FacetParams.FACET_DATE_GAP);
        ...
        final String[] othersP =
                params.getFieldParams(key,FacetParams.FACET_DATE_OTHER);
